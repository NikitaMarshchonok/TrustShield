...
def test_latency_latest_endpoint() -> None:
    response = client.get("/latency/latest")
    assert response.status_code == 200
    body = response.json()
    assert body["status"] in {"ok", "missing"}


def test_alerts_latest_endpoint() -> None:
...
def test_drift_latest_endpoint() -> None:
...
def test_decision_mix_latest_endpoint() -> None:
    response = client.get("/decision-mix/latest")
    assert response.status_code == 200
    body = response.json()
    assert body["status"] in {"ok", "missing"}
...
def test_reports_generate_all_endpoint() -> None:
